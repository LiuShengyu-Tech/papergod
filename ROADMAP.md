...
| Agent 建议、diff、接受/拒绝 | 已实现 | Mock/Codex/OpenCode 统一生成结构化建议，逐条接受或拒绝 |
| Codex/OpenCode CLI | 已实现 | 非交互、安全参数、结构化输出、超时/取消与审计记录 |
...
| 全文意见编排与任务分配 | 已实现 | Mock/Codex/OpenCode 结构化编排、原子意见、节点分配、依赖/冲突图与显式决策 |
...
| 自动化测试 | 已实现 | 90 项 API、结构、CLI、Agent 配置/Prompt 预览、Demo 初始化、库、评审、修订、生成、安全和编译测试通过 |
